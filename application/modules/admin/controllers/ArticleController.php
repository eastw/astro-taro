...
class Admin_ArticleController extends Zend_Controller_Action{
	...
	public function indexAction(){
		...
		$query = $this->service->buildArticlesQuery();
	...
	public function addAction(){
		...
		$form->fillTags($this->tags->getAllArticleTags());
		...
		if($this->getRequest()->isPost()){
			...
			if($form->isValid($formData)){
				...
				unset($session->tags);
				$this->service->addArticle($validData);
				$this->navigation->refreshNavigation();
			...
			if($form->isValid($formData)){
				...
				foreach ($adapter->getFileInfo() as $file) {
					if(!empty($file['name'])){
						...
						$adapter->receive($file['name']);
						$realPath = realpath(dirname('.')).DIRECTORY_SEPARATOR.'files'.DIRECTORY_SEPARATOR.'articles'.DIRECTORY_SEPARATOR.$article['image'];
				...
				$validData['tags'] = $session->tags;
				$this->service->saveArticle($validData,$article['id']);
				$this->navigation->refreshNavigation();
				$this->profileService->refreshFavoriteLink($article['id'], 'article');
				$this->redirect((!empty($page))?'/admin/article/index/page/'.$page :'/admin/article');
			}else{
				$form->img_note->setValue('<img src="/files/articles/'.$article['image'].'" />');
				$form->fillTagsplace();
				$form->populate($formData);
